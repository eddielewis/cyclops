...
class Stitcher:
    def set_params_from_file(self, fn):
        # Loads homography matrices and camera layout from a NumPY .npz file.
    ...
    def stitch_h(self, frame_dict, estimate_H):
        """
        Horizontally stitches a set of images into one image.
        Expects a dict of str(camera_id) and np.array(frame),
        and boolean value for whether homography should be done or not.
        Returnes a stitched image of the image set.
        """
        # Cannot stitch if the matrices or layout is not known
        if self.h_matrices is None or self.camera_layout is None:
            return None
        stitched_img = frame_dict[self.camera_layout[0]]
        # Iterates through the image pairs according to the camera layout list
        for i in range(len(self.camera_layout)-1):
            ...
            if estimate_H:
                ...
                # Finds the homography matrix for the image pair
...
def matrix_id(cam_a_id, cam_b_id):
    """
    Used to generate a homography matrix key from camera ids.
    Expects str(camera_id)
    Returns str(matrix_key)
    """
    return cam_a_id + "_" + cam_b_id


def estimate_homography(images, ratio=0.75, reproj_thresh=4.0):
    # Taken from https://www.pyimagesearch.com/2016/01/25/real-time-panorama-and-image-stitching-with-opencv/
...
def remove_border(stitched_img):
    """
    Finds contours in the image, calculates their areas and removes all but
    the largest one from the image. This reduces the number of image fragments
    caused by stitching.
    Expects a stitched image.
    Returns the image with any excess removed.
    """
...
def stitch_pair(images, H, w, h):
    """
    Uses homography matrix to warp the image perspective,
    appends the other image in the pair and removes black regions
    from the image using contours.
    Expects two images, the homography matrix that relates them,
    the width and height of the images.
    Returns a stitched image.
    """
    img_a, img_b = images
...
def calc_h_matrices(frame_dict):
    """
    Calculates homography matrices for each frame pair.
    Expects a dict of str(camera_id) and np,array(frame)
    Returns dict of str(camera_id) and np.array(h_matrix)
    """
    h_matrices = {}
...
# Defines the camwera layout so the images can be stitched in the correct order
